...
vat_pham_phu_kien=[0,0,0]
'''
Day la noi de chu thich phu kieen dang luu, 0 la de bieu thi van chua mua phu kien do

[0]: sao đỏ cho xe(setnv1)
[1]: khẩu trang sinh viên(setnv5)
[2]: chưa có, mẫu cách trình bày cho dễ nhìn thui
'''
...
Thang = int(noi_dung_file[3])
Thua = int(noi_dung_file[4])
Tongtiencuoc = int(noi_dung_file[5])
Tienthang = None	
thang_thua = 1
...
def phu_kien():
    def sao_do():
        def ap_dung():
            global setnv_ver_rc1
            setnv_ver_rc1="mod1"
            phu_kien()
        def mua():
            global vat_pham_phu_kien
            vat_pham_phu_kien[0]=1
            sao_do()
        menu.clear()
        global vat_pham_phu_kien
        if(vat_pham_phu_kien[0]==0):
        ...
        if(vat_pham_phu_kien[0]==1):
            menu.add_label("Bạn có muốn áp dụng không?")
            menu.add_button('Có', ap_dung)
            menu.add_button('Không',phu_kien)
    def mask():
        def ap_dung():
            global setnv_ver_rc5
            setnv_ver_rc5="mod1"
            phu_kien()
        def mua():
            global vat_pham_phu_kien
            vat_pham_phu_kien[1]=1
            mask()
        menu.clear()
        global vat_pham_phu_kien
        if(vat_pham_phu_kien[1]==0):
            menu.add_label("San pham nay co gia")
            menu.add_label(gia_sp[0])
            menu.add_label("Bạn có muốn mua sản phẩm này không?")
            menu.add_button('Có',mua)
            menu.add_button('Không',phu_kien)
        if(vat_pham_phu_kien[1]==1):
            menu.add_label("Bạn có muốn áp dụng không?")
            menu.add_button('Có', ap_dung)
            menu.add_button('Không',phu_kien)
    def reset_setnv1():
...
def dat_cuoc(value,choice):
    if(choice == 1000):
        ...
        Tienthang = int(choice)*1000
    global Tongtiencuoc
    Tongtiencuoc = Tongtiencuoc + Tienthang
#-------------------------------


def doi_bua():
    menu.clear()
    menu.add_label('Bạn có muốn đổi số tiền thắng được thành bùa ko???')
    menu.add_button('Có',doi)
    menu.add_button('Không',choi_lai)

def doi():
    menu.clear()
    menu.add_label('Số tiền bạn thắng được là: {0}'.format(Tienthang))
    menu.add_label('1 bùa có giá là 25,000 VNĐ')
    menu.add_label('Đổi không')
    menu.add_button('Đổi',codoi)
    menu.add_button('Không',choi_lai)
...
def main_game():
    ...
    menu.add_selector('Đặt cược', [('None',0),('1000', 1), ('2000', 2), ('5000', 5), ('10000',10), ('20000',20),('50000',50),('100000',100),('200000',200),('500000',500),('Toàn bộ',1000)], onchange=dat_cuoc,align=pygame_menu.locals.ALIGN_LEFT)
...
def history():
    menu.clear()
    menu.add_label("Thắng: {0} lần ".format(Thang))
    menu.add_label("Thua: {0} lần ".format(Thua))
    menu.add_label("Số tiền đã cược: {0} VNĐ ".format(Tongtiencuoc))
    menu.add_button("Ok",chuan_bi)
...
def save_game():
    ...
    noi_dung_file[3] = str(Thang)+'\n'
    noi_dung_file[4] = str(Thua)+'\n'
    noi_dung_file[5] = str(Tongtiencuoc)+'\n'
    
    file = open('user.txt','w')
    file.write(noi_dung_file[0])
    file.write(noi_dung_file[1])
    file.write(noi_dung_file[2])
    file.write(noi_dung_file[3])
    file.write(noi_dung_file[4])
    file.write(noi_dung_file[5])
    da_luu()
...
def choi_lai():
    menu.clear()
    menu.add_label("Bạn có muốn chơi tiếp ko???")
    menu.add_button("Có",bat_dau)
    menu.add_button("Không",save_game)
...
def chuan_bi():
    ...
    menu.add_button("Lịch sử chơi",history,align=pygame_menu.locals.ALIGN_LEFT)
